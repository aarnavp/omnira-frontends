 ...
 * the mobile diagram (PROMPT §8.7 — restructured, not shrunk) and doubles
 * as the desktop keyboard-navigable / screen-reader fallback for the React
 * Flow canvas.
 */
export function PlaneFlowList({
  ...
  return (
    <Accordion
      ...
      className="relative"
    >
      <div className="pointer-events-none absolute top-8 bottom-8 left-4 w-px bg-(--color-border)" aria-hidden />
      {planes.map((plane, index) => (
        <AccordionItem key={plane.id} value={plane.id} className="relative border-b border-(--color-border) pl-11 last:border-b-0">
          <span
            aria-hidden
            className={cn(
              "absolute top-4 left-0 flex h-8 w-8 items-center justify-center rounded-(--radius-full) border font-mono text-xs",
              plane.isPrimary
                ? "border-(--color-accent) bg-(--color-signal-50) text-(--color-accent)"
                : "border-dashed border-(--color-baseline) bg-(--color-surface) text-(--color-baseline-text)",
            )}
          >
            {index + 1}
          </span>
          <AccordionTrigger>
            <span className="flex flex-col gap-0.5">
              ...
              <span className="text-sm font-semibold text-(--color-text)">{plane.name}</span>
            </span>
          </AccordionTrigger>
          <AccordionContent>
            <p>{plane.detail}</p>
