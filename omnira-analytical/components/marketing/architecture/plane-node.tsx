...
const handleClass = "!h-2 !w-2 !border-none !bg-(--color-border-strong)";
  ...
  return (
    <>
      ...
      <button
        ...
        aria-pressed={active}
        className={cn(
          "w-[188px] rounded-(--radius-lg) border bg-(--color-surface-raised) px-4 py-3 text-left shadow-(--shadow-hairline) transition-all duration-200",
          !plane.isPrimary && "border-dashed",
          active ? "border-(--color-accent) shadow-(--shadow-hairline-raised)" : "border-(--color-border)",
          dimmed ? "opacity-35" : "opacity-100",
        )}
      >
        <p className="font-mono text-[10px] uppercase tracking-wider text-(--color-text-faint)">{plane.role}</p>
        <p className="mt-1 text-sm leading-snug font-semibold text-(--color-text)">{plane.name}</p>
        <span
          className={cn(
            "mt-2 inline-flex items-center gap-1 rounded-(--radius-full) px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide",
            plane.isPrimary ? "bg-(--color-signal-100) text-(--color-accent)" : "bg-(--color-slate-100) text-(--color-baseline-text)",
          )}
        >
