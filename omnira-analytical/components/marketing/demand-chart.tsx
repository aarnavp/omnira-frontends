...
/** Demand vs. provisioned-capacity chart for §01 Problem — makes the waste
 * of always-on provisioning visible in one image. Illustrative/conceptual,
 * not a measurement of a real deployment. */
export function DemandChart() {
  const { data, error, isLoading, refetch } = useDemandModel();

  if (isLoading) {
    return (
      <div className="flex h-[320px] flex-col gap-3 rounded-(--radius-lg) border border-(--color-border) bg-(--color-surface-raised) p-5">
  ...
  return (
    <motion.div
      ...
      className="rounded-(--radius-lg) border border-(--color-border) bg-(--color-surface-raised) p-5"
    >
      ...
      <div className="h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data.points} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <XAxis
              dataKey="label"
              tick={{ fontSize: 11, fill: "var(--color-text-faint)", fontFamily: "var(--font-mono)" }}
              tickLine={false}
