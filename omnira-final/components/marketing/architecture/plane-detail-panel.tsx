...
export function PlaneDetailPanel({ plane }: { plane: ArchitecturePlane }) {
  return (
    <Card className="p-5" aria-live="polite">
      <p className="font-mono text-[11px] uppercase tracking-wider text-(--color-ink-faint)">{plane.role}</p>
      <h3 className="mt-1 text-lg font-semibold text-(--color-ink)">{plane.name}</h3>
      <p className="mt-3 text-sm leading-relaxed text-(--color-ink-muted)">{plane.detail}</p>
