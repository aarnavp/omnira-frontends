 ...
 * instead of just stating it. Stops at the final step — no idle looping. */
export function ScalingCounter({ series }: { series: ScalingPoint[] }) {
  ...
  return (
    <div ref={ref} className="rounded-(--radius-lg) border border-(--color-hairline) bg-(--color-panel) p-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-6" aria-live="polite">
        <div>
          <p className="text-sm text-(--color-ink-muted)">Devices in the network</p>
          <p className="mt-1 font-mono text-2xl font-medium text-(--color-ink) sm:text-3xl">
            <CountUp value={point.devices} format="number" />
          </p>
        </div>
        <div>
          <p className="text-sm text-(--color-ink-muted)">Available capacity (TFLOPS)</p>
      ...
      <div className="mt-5 h-1.5 w-full overflow-hidden rounded-(--radius-full) bg-(--color-panel-sunken)">
        <div
