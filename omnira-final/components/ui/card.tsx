...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-(--radius-lg) bg-(--color-panel) shadow-(--shadow-hairline)",
        hoverLift &&
  ...
  return (
    <div className={cn("flex items-start justify-between gap-4 p-5 pb-3", className)}>
      <div className="min-w-0">
        {eyebrow ? (
          <p className="font-mono text-[11px] uppercase tracking-wider text-(--color-ink-faint)">
            {eyebrow}
          </p>
        ) : null}
        <h3 className="mt-0.5 text-base font-semibold text-(--color-ink)">{title}</h3>
        {description ? (
